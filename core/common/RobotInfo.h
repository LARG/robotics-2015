...
// class so that lua can access it properly
class Camera {
public:
  enum Type {
    TOP = 0,
...
// camera param struct
struct CameraParams {
  int kCameraAutoWhiteBalance;
  int kCameraExposureAuto;
  int kCameraBacklightCompensation;

  int kCameraBrightness;
  int kCameraContrast;
  int kCameraSaturation;
  int kCameraHue;
  int kCameraExposure;
  int kCameraGain;
  int kCameraSharpness;
};
...
struct ImageParams {
  ...
  int size;
  int rawSize;
  int defaultHorizontalStepScale;
  int defaultVerticalStepScale;
  int factor;
  float origFactor;
