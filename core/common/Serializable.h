...
class Serializable {
  public:
    ...
    friend void operator>>(std::istream& is, Serializable& s) {
      s.deserialize(is);
    }
  ...
  protected:
    ...
    void serializeMember(const Serializable*& member) const {
      if(member) {
    ...
    void serializeMember(const Serializable* const& member) const {
      if(member) {
        serializeMember(*member);
      } else {
        CHECK_BUFFER;
...
#define DESERIALIZE_CONTAINER(member,T) { \
    ...
    // Can't use Serializable*& because the compiler can't determine the type to instantiate
    template<typename T>
      void deserializeMember(T*& member) {
        CHECK_BUFFER;
    ...
    void deserializeMember(Serializable& member) {
      CHECK_BUFFER;
