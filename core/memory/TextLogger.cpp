...
TextLogger::TextLogger(const char *filename, bool appendUniqueId) : frameInfo(NULL) {
  enabled = false;
  toolSimMode = false;
  if (filename) {
...
void TextLogger::writeDebug(int loglevel, int frame, int moduleType, char *msg ) {
  ...
  if ( toolSimMode ){
    ...
    snprintf(buffer, MESSAGE_SIZE, "loglev %d: frame %d: module %d: %s", loglevel, frame, moduleType, msg );
...
void TextLogger::log(int logLevel, int moduleType, const char* format, ...) {
  ...
  int l=vsnprintf( temp, MESSAGE_SIZE, format, args );
...
void TextLogger::logFromLocalization(int logLevel, const char* format, ...) {
  CHECK_LOGGER_LEVELS
  va_list args;
  char temp[MESSAGE_SIZE];
  va_start( args, format );
...
void TextLogger::generateUniqueFileName(char *fileName, const char *prefix, const char *ext) {
  ...
  std::string logFile = base + std::string(prefix) + "_%y_%m_%d-%H_%M_%S/frames." + std::string(ext);
  time_t rawtime;
  struct tm *timeInfo;
  time(&rawtime);
