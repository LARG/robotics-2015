...
void InterpreterModule::updatePercepts() {
  memcpy(&joint_values_[0], joint_angles_->values_, NUM_JOINTS * sizeof(float));
  memcpy(&joint_stiffness_[0], joint_angles_->stiffness_, NUM_JOINTS * sizeof(float));
  memcpy(&sensor_values_[0], sensors_->values_, NUM_SENSORS * sizeof(float));
}
