...
AnnotationWidget::AnnotationWidget(QWidget* parent) : QWidget(parent), selectedAnnotation_(0) {
    ...
    selectionStrings[RectangleSelectionType] = "Rectangle";
    selectionStrings[PolygonSelectionType] = "Polygon";
    selectionStrings[EllipseSelectionType] = "Ellipse";
    selectionStrings[ManualSelectionType] = "Manual";
...
void AnnotationWidget::selected(Selection* selection){
    ...
    std::map<std::string, Annotation* >::iterator it = annotations_.find(name);
...
std::vector<Annotation*> AnnotationWidget::getAnnotations() {
    ...
    for(int i = 0; i < count; i++){
        ...
        Annotation* annotation = aitem->getAnnotation();
...
void AnnotationWidget::loadChoices(Annotation* annotation) {
...
void AnnotationWidget::loadSelections(Annotation* annotation){
    selectionList->clear();
...
void AnnotationWidget::selectionBoxIndexChanged(const QString& text) {
    if(text == "Rectangle")
        emit selectionTypeChanged(RectangleSelectionType);
    else if (text == "Polygon")
        emit selectionTypeChanged(PolygonSelectionType);
    else if (text == "Ellipse")
        emit selectionTypeChanged(EllipseSelectionType);
    else if (text == "Manual")
        emit selectionTypeChanged(ManualSelectionType);
}
...
void AnnotationWidget::handleNewLogLoaded(Log* log){
    log_ = log;
    ...
    std::vector<Annotation*> annotations = group.getAnnotations();
...
void AnnotationWidget::deleteAnnotation(){
    ...
    AnnotationListWidgetItem* aitem = (AnnotationListWidgetItem*)item;
    Annotation* annotation = aitem->getAnnotation();
    std::map<std::string, Annotation*>::iterator it = annotations_.find(annotation->getName());
...
bool AnnotationWidget::filterAnnotation(Annotation* annotation){
