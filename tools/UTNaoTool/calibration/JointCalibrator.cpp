...
JointCalibrator::JointCalibrator() : cache_(NULL), vblocks_(NULL), params_(NULL), processor_(NULL), cal_(NULL), thread_(NULL), memory_(NULL) {
  memory_ = new Memory(false, MemoryOwner::TOOL_MEM, 0, 1);
...
void JointCalibrator::setMemory(Memory* memory) {
...
void JointCalibrator::takeSamples(Log* log) {
  ...
  for(int i = 0; i < log->size(); i++) {
    ...
    printf("found %i corners in frame %i\n", m.corners.size(), i);
...
JointCalibrator::Measurement JointCalibrator::takeSample(Memory* frame) {
