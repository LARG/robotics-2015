...
#include <tool/annotations/AnnotationGroup.h>
...
void UTMainWnd::loadLog(const char *directory) {
  ...
  memory_log_ = new Log(directory, config_.logStart, config_.logEnd);
...
void UTMainWnd::runCore() {
  ...
  if (!coreAvailable_) {
    ...
    memory_log_->enableCache() = !config_.onDemand;
...
void UTMainWnd::runStream() {
  runningCore_ = false;
  isStreaming_ = true;
  stream_memory_ = Memory(false,MemoryOwner::TOOL_MEM, 0, 1);
